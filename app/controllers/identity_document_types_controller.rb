class IdentityDocumentTypesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to identity_document_types_path, status: :see_other, notice: "Identity document type was successfully destroyed." }
  ...
  private
    ...
    def set_identity_document_type
      @identity_document_type = IdentityDocumentType.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def identity_document_type_params
      params.expect(identity_document_type: [ :name, :short_name, :display_order, :active ])
    end
end
